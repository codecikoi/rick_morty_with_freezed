...
class _SearchPageState extends State<SearchPage> {
  ...
  void initState() {
    if(_storage.runtimeType.toString().isEmpty) {
      if (_currentResults.isEmpty) {
        context.read<CharacterBloc>().add(CharacterEvent.fetch(name: '', page: 1),);
      }
    }
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        Padding(padding: EdgeInsets.only(top: 15, bottom: 15, left: 16, right: 16,),
        child: TextField(
        ...
        Expanded(child: state.when(loading: () {
          if (!_isPagination) {
            return Center(
            child: Row(
  ...
  Widget _customListView(List<Results> currentResults) {
    return SmartRefresher(controller: refreshController,
      enablePullUp: true,
      ...
      child: ListView.separated(itemBuilder: (context, index) {final results = currentResults[index]; return Padding(padding: EdgeInsets.only(right: 16, left: 16, top: 3, bottom: 3,), child: CustomListTile(results: results,),);}, separatorBuilder: (_, index) => SizedBox(height: 5,), itemCount: currentResults.length),
